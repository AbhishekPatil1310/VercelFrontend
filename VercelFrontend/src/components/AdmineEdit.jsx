...
import AdvertiserAds from './AdminAds'; // Make sure this file matches your actual component filename
...
export default function EditUserModal({ user, onClose, onUpdated }) {
  ...
  return (
    <>
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-gradient-to-br from-indigo-900/40 via-purple-900/30 to-blue-900/40 backdrop-blur-sm px-4">
        <div className="w-full max-w-lg rounded-xl bg-white/70 backdrop-blur-md shadow-2xl border border-white/30 p-6 sm:p-8">
          ...
          <button
            ...
            className={`w-full mt-4 ${isBanned
          ...
          {form.role === 'advertiser' && (
            <div className="mt-4">
              <button
                ...
                className="w-full bg-blue-500 hover:bg-blue-600 text-white font-medium px-4 py-2 rounded-lg shadow-md transition"
              >
                📢 View Ads
              </button>
            </div>
      ...
      {/* Ads Modal */}
      {showAdsModal && (
        <AdvertiserAds advertiserId={user._id} onClose={() => setShowAdsModal(false)} />
